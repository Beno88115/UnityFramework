...
namespace Tacticsoft
{
    ...
    public class TableView : MonoBehaviour
    {
        ...
        public ITableViewCellSource cellSource
        {
            get { return this.m_cellSource; }
            set { this.m_cellSource = value; }
        }
        ...
        public void MoveCell(int index, bool isNeedAddCell)
        {
            int lastIndex = this.visibleRowRange.from + this.visibleRowRange.count - 1;
            if (isNeedAddCell == true)
            {
                lastIndex++;
                this.m_visibleRowRange.count++;
                this.AddRow(lastIndex, true);
                UpdatePaddingElements();
            }

            this.RefreshCell(index, lastIndex);
            this.m_visibleRowRange.count--;
        }

        public void SetCellReuse(TableViewCell cell)
        {
        ...
        {
            this.m_rowHeights = new float[this.dataSource.GetNumberOfRowsForTableView(this)];
            ...
            this.m_scrollRect.content.sizeDelta = new Vector2(this.m_scrollRect.content.sizeDelta[0],
               this.GetCumulativeRowHeight(this.m_rowHeights.Length - 1));
            //RecalculateVisibleRowsFromScratch();
            m_requiresRefresh = true;
        }

        private void RefreshCell(int fromIndex, int toIndex)
        {
            for (int i = fromIndex; i <= toIndex; i++)
            {
                if (this.m_visibleCells.ContainsKey(i + 1) == false)
                {
                    continue;
                }

                this.m_visibleCells[i] = this.m_visibleCells[i + 1];
                this.cellSource.RefreshTableView(this.m_visibleCells[i], i);
            }

            this.m_visibleCells.Remove(toIndex);
        }
        ...
        private ITableViewCellSource m_cellSource;
