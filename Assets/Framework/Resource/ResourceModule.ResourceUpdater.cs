...
namespace GameFramework.Resource
{
    internal sealed partial class ResourceModule : GameFrameworkModule, IResourceModule
    {
        ...
        private sealed partial class ResourceUpdater
        {
            ...
            private IDownloadModule m_DownloadModule;
            ...
            public ResourceUpdater(ResourceModule resourceModule)
            {
            ...
            public void Update(float elapseSeconds, float realElapseSeconds)
            {
                ...
                {
                    if (m_DownloadModule.FreeAgentCount > 0)
                    {
                        UpdateInfo updateInfo = m_UpdateWaitingInfo[0];
                        m_UpdateWaitingInfo.RemoveAt(0);
                        m_DownloadModule.AddDownload(updateInfo.ResourcePath, Utility.Path.GetRemotePath(m_ResourceModule.m_UpdatePrefixUri, Utility.Path.GetResourceNameWithCrc32AndSuffix(updateInfo.ResourceName.FullName, updateInfo.HashCode)), updateInfo);
                        m_UpdatingCount++;
                    }
                }
                else if (m_UpdatingCount <= 0)
                {
                    ...
                    Utility.Path.RemoveEmptyDirectory(m_ResourceModule.m_ReadWritePath);
            ...
            public void Shutdown()
            {
                ...
                {
                    m_DownloadModule.DownloadStart -= OnDownloadStart;
                    m_DownloadModule.DownloadUpdate -= OnDownloadUpdate;
                    m_DownloadModule.DownloadSuccess -= OnDownloadSuccess;
                    m_DownloadModule.DownloadFailure -= OnDownloadFailure;
                }
            ...
            /// 设置下载管理器。
            /// </summary>
            /// <param name="downloadModule">下载管理器。</param>
            public void SetDownloadModule(IDownloadModule downloadModule)
            {
            ...
            public void CheckResourceComplete(bool needGenerateReadWriteList)
            {
                ...
                if (m_ResourceModule.UpdateFileCacheLength < maxLength)
                {
                    m_ResourceModule.UpdateFileCacheLength = (maxLength / OneMegaBytes + 1) * OneMegaBytes;
                }
            }
            ...
            {
                string file = Utility.Path.GetCombinePath(m_ResourceModule.m_ReadWritePath, Utility.Path.GetResourceNameWithSuffix(ResourceListFileName));
                ...
                try
                {
                    ...
                    using (BinaryWriter binaryWriter = new BinaryWriter(fileStream, Encoding.UTF8))
                    {
                        ...
                        binaryWriter.Write(m_ResourceModule.m_ReadWriteResourceInfos.Count);
                        foreach (KeyValuePair<ResourceName, ReadWriteResourceInfo> i in m_ResourceModule.m_ReadWriteResourceInfos)
                        {
            ...
            private void OnDownloadStart(object sender, DownloadStartEventArgs e)
            {
                ...
                {
                    m_DownloadModule.RemoveDownload(e.SerialId);
            ...
            private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
            {
                ...
                using (FileStream fileStream = new FileStream(e.DownloadPath, FileMode.Open, FileAccess.ReadWrite))
                {
                    ...
                    if (m_ResourceModule.UpdateFileCacheLength < length)
                    {
                        m_ResourceModule.UpdateFileCacheLength = (length / OneMegaBytes + 1) * OneMegaBytes;
                    }
                    ...
                    while (count > 0)
                    {
                        int bytesRead = fileStream.Read(m_ResourceModule.m_UpdateFileCache, offset, count);
                        if (bytesRead <= 0)
                        {
                    ...
                    if (!zip)
                    {
                        ...
                        {
                            Utility.Encryption.GetQuickSelfXorBytes(m_ResourceModule.m_UpdateFileCache, m_CachedHashBytes);
                        }
                        else if (updateInfo.LoadType == LoadType.LoadFromMemoryAndDecrypt)
                        {
                            Utility.Encryption.GetSelfXorBytes(m_ResourceModule.m_UpdateFileCache, m_CachedHashBytes, length);
                        }

                        Array.Clear(m_CachedHashBytes, 0, CachedHashBytesLength);
                    }

                    int hashCode = Utility.Converter.GetInt32(Utility.Verifier.GetCrc32(m_ResourceModule.m_UpdateFileCache, 0, length));
                    ...
                    if (zip)
                    {
                        try
                        {
                            if (m_ResourceModule.m_DecompressCache == null)
                            {
                                m_ResourceModule.m_DecompressCache = new MemoryStream();
                            }

                            m_ResourceModule.m_DecompressCache.Position = 0L;
                            m_ResourceModule.m_DecompressCache.SetLength(0L);
                            if (!Utility.Zip.Decompress(m_ResourceModule.m_UpdateFileCache, 0, length, m_ResourceModule.m_DecompressCache))
                            {
                                fileStream.Close();
                            ...
                            if (m_ResourceModule.m_DecompressCache.Length != updateInfo.Length)
                            {
                                fileStream.Close();
                                string errorMessage = Utility.Text.Format("Resource length error, need '{0}', downloaded '{1}'.", updateInfo.Length.ToString(), m_ResourceModule.m_DecompressCache.Length.ToString());
                                DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                                OnDownloadFailure(this, downloadFailureEventArgs);
                                ReferencePool.Release(downloadFailureEventArgs);
                                return;
                            ...
                            m_ResourceModule.m_DecompressCache.Position = 0L;
                            int bytesRead = 0;
                            while ((bytesRead = m_ResourceModule.m_DecompressCache.Read(m_ResourceModule.m_UpdateFileCache, 0, m_ResourceModule.m_UpdateFileCache.Length)) > 0)
                            {
                                fileStream.Write(m_ResourceModule.m_UpdateFileCache, 0, bytesRead);
                            }
                        }
                        catch (Exception exception)
                        {
                ...
                if (m_ResourceModule.m_ResourceInfos.ContainsKey(updateInfo.ResourceName))
                {
                    throw new GameFrameworkException(Utility.Text.Format("Resource info '{0}' is already exist.", updateInfo.ResourceName.FullName));
                }

                m_ResourceModule.m_ResourceInfos.Add(updateInfo.ResourceName, new ResourceInfo(updateInfo.ResourceName, updateInfo.LoadType, updateInfo.Length, updateInfo.HashCode, false));

                if (m_ResourceModule.m_ReadWriteResourceInfos.ContainsKey(updateInfo.ResourceName))
                {
                ...
                m_ResourceModule.m_ReadWriteResourceInfos.Add(updateInfo.ResourceName, new ReadWriteResourceInfo(updateInfo.LoadType, updateInfo.Length, updateInfo.HashCode));
