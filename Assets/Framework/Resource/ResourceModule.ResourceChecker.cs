...
namespace GameFramework.Resource
{
    internal sealed partial class ResourceModule : GameFrameworkModule, IResourceModule
    {
        ...
        private sealed partial class ResourceChecker
        {
            ...
            public ResourceChecker(ResourceModule resourceModule)
            {
            ...
            public void CheckResources(string currentVariant)
            {
                ...
                m_ResourceModule.m_ResourceHelper.LoadBytes(Utility.Path.GetRemotePath(m_ResourceModule.m_ReadWritePath, Utility.Path.GetResourceNameWithSuffix(VersionListFileName)), ParseVersionList);
                m_ResourceModule.m_ResourceHelper.LoadBytes(Utility.Path.GetRemotePath(m_ResourceModule.m_ReadOnlyPath, Utility.Path.GetResourceNameWithSuffix(ResourceListFileName)), ParseReadOnlyList);
                m_ResourceModule.m_ResourceHelper.LoadBytes(Utility.Path.GetRemotePath(m_ResourceModule.m_ReadWritePath, Utility.Path.GetResourceNameWithSuffix(ResourceListFileName)), ParseReadWriteList);
            }
            ...
            private void RefreshCheckInfoStatus()
            {
                ...
                foreach (KeyValuePair<ResourceName, CheckInfo> checkInfo in m_CheckInfos)
                {
                    ...
                    if (ci.NeedRemove)
                    {
                        ...
                        string path = Utility.Path.GetCombinePath(m_ResourceModule.m_ReadWritePath, Utility.Path.GetResourceNameWithSuffix(ci.ResourceName.FullName));
                        File.Delete(path);

                        if (!m_ResourceModule.m_ReadWriteResourceInfos.ContainsKey(ci.ResourceName))
                        {
                            throw new GameFrameworkException(Utility.Text.Format("Resource '{0}' is not exist in read-write list.", ci.ResourceName.FullName));
                        }

                        m_ResourceModule.m_ReadWriteResourceInfos.Remove(ci.ResourceName);
                    }
                }
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(memoryStream, Encoding.UTF8))
                    {
                        ...
                        if (listVersion == 0)
                        {
                            ...
                            for (int i = 0; i < resourceCount; i++)
                            {
                                ...
                                ResourceName resourceName = new ResourceName(name, variant);
                                if (m_ResourceModule.m_ReadWriteResourceInfos.ContainsKey(resourceName))
                                {
                                    throw new GameFrameworkException(Utility.Text.Format("Read-write resource info '{0}' is already exist.", resourceName.FullName));
                                }

                                m_ResourceModule.m_ReadWriteResourceInfos.Add(resourceName, new ReadWriteResourceInfo(loadType, length, hashCode));
                            }
                        }
                        else
                        {
            ...
            private void ProcessResourceInfo(ResourceName resourceName, LoadType loadType, int length, int hashCode, bool storageInReadOnly)
            {
                ...
                m_ResourceModule.m_ResourceInfos.Add(resourceName, new ResourceInfo(resourceName, loadType, length, hashCode, storageInReadOnly));
            }
        }
    }
}
