...
namespace GameFramework.Resource
{
    internal sealed partial class ResourceModule : GameFrameworkModule, IResourceModule
    {
        ...
        private sealed class VersionListProcessor
        {
            ...
            public VersionListProcessor(ResourceModule resourceModule)
            {
            ...
            {
                if (string.IsNullOrEmpty(m_ResourceModule.m_ReadWritePath))
                {
                ...
                string versionListFileName = Utility.Path.GetCombinePath(m_ResourceModule.m_ReadWritePath, Utility.Path.GetResourceNameWithSuffix(VersionListFileName));
                ...
                try
                {
                    ...
                    using (BinaryReader binaryReader = new BinaryReader(fileStream, Encoding.UTF8))
                    {
                        ...
                        if (listVersion == 0)
                        {
                            ...
                            applicableGameVersion = m_ResourceModule.GetEncryptedString(binaryReader, encryptBytes);
            ...
            public void UpdateVersionList(int versionListLength, int versionListHashCode, int versionListZipLength, int versionListZipHashCode)
            {
                ...
                string localVersionListFilePath = Utility.Path.GetCombinePath(m_ResourceModule.m_ReadWritePath, versionListFileName);
                string latestVersionListFileUri = Utility.Path.GetRemotePath(m_ResourceModule.m_UpdatePrefixUri, latestVersionListFileName);
                m_DownloadModule.AddDownload(localVersionListFilePath, latestVersionListFileUri, this);
            }
            ...
            private void OnDownloadSuccess(object sender, DownloadSuccessEventArgs e)
            {
                ...
                using (FileStream fileStream = new FileStream(e.DownloadPath, FileMode.Open, FileAccess.ReadWrite))
                {
                    ...
                    try
                    {
                        ...
                        if (m_ResourceModule.m_DecompressCache.Length != m_VersionListLength)
                        {
                            fileStream.Close();
                            string errorMessage = Utility.Text.Format("Latest version list length error, need '{0}', downloaded '{1}'.", m_VersionListLength.ToString(), m_ResourceModule.m_DecompressCache.Length.ToString());
                            DownloadFailureEventArgs downloadFailureEventArgs = DownloadFailureEventArgs.Create(e.SerialId, e.DownloadPath, e.DownloadUri, errorMessage, e.UserData);
                            OnDownloadFailure(this, downloadFailureEventArgs);
                            ReferencePool.Release(downloadFailureEventArgs);
                            return;
