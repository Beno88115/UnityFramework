...
TEMPLATE_CSHARP_MANAGER = 'ConfigManagerConfigNameList.cs'
TEMPLATE_ITEM           = 'ConfigRows.cs'
TEMPLATE_SUB_ITEM       = 'ConfigRow.cs'
...
CSHARP_JSON_TYPE    = 'JSONArray'

LEFT_BRACE  = '{'
RIGHT_BRACE = '}'

CONFIG_MANAGER_FIELD_DECLARE = '\n\tConfigTable<{name}> {instance} = new ConfigTable<{name}>();'
...
class CSharpGenerate(Base):
    ...
    def __init__(self):
        Base.__init__(self)
        self._make_dirs(WIRTE_DIRECTORY)
    ...
    def get_table_structure(self, sheet_name):
        ...
        for column_index in range(table.ncols):
            ...
            elif column_type == COLUMN_JSON_TYPE:
                code_type = CSHARP_JSON_TYPE
    ...
    def get_config_item_filed(self, column_type, column_title):
        return str.format('\n\tpublic {} {} {} get; private set; {}', column_type, self.change_to_code_name(column_title), '{', '}')
    ...
    def get_config_item_assigment(self, column_type, column_title, column_index):
        ...
        elif column_type == CSHARP_JSON_TYPE:
            assign_clause += " as JSONArray;"
        else:
            assign_clause += ";"
    ...
    def get_config_item_class_name(self, output_name):
        return str.format('Config{}Row', self.change_to_code_name(output_name))
    ...
    def get_config_manager_field_name(self, output_name):
        return str.format('m_Config{}Table', self.change_to_code_name(output_name))
    ...
    def get_config_info(self):
        ...
        for row in self._output_config_table:
            ...
            if len(config_name_list) > 0:
                config_name_list += ", "
            config_name_list += str.format('"{}"', self.change_to_code_name(output_name))
            # config_name_array.append(output_name)

            config_items += self.get_config_sub_item(output_name, column_array)
            config_items += '\n\n'

        return config_name_list, config_items

    # ========> 外部调用接口
    # 生成ConfigManager.cs文件
    def export_config_manager(self, config_name_list):
        config_manager = self.try_open_file(TEMPLATE_CSHARP_MANAGER)
        config_manager = config_manager.format(config_name_list = config_name_list, left = LEFT_BRACE, right = RIGHT_BRACE)
